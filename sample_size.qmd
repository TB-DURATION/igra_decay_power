...
```{r}
out <- decay_rate_power(p0 = .3, p1 = .1, t = 10,
                        t_entry = function() runif(200, max = 10), N = 1000)
```
...
```{r include = FALSE}
samples_sizes <- seq(10, 1000, 10)
powers <- readRDS("/Users/choisy/Library/CloudStorage/OneDrive-OxfordUniversityClinicalResearchUnit/GitHub/tb-duration/igra_decay_power/powers.RDS")
```

Let's visualize it:

```{r}
mean_powers <- map_dbl(powers, mean)
...
smoothed <- lowess(samples_sizes, mean_powers, .1)
with(smoothed, lines(x, y, col = 4, lwd = 2))
```

Looking for the number of samples that give 80% power:

```{r}
power2n <- function(power, smoothed = smoothed, ...) {
  optimize(function(xout) with(smoothed, abs(approx(x, y, xout, ...)$y - power)),
           c(0, 1000))$minimum
}
```
